...
export default function PrayerEditModal({ open, onClose, id }: modalProps) {
    ...
    const onSubmitCreatePrayerHandler = async (data: TPrayerZodFormSchema, id: string) => {
        if (!data.userDate
            || data.title === ''
            || data.description === ''
        ...
        await axiosInstanceClient.put(`/prayers/${id}`,
            {
    ...
    const fetchData = async () => {
        try {
            const { data } = await axiosInstanceClient.get(`/prayers/${id}`, {
                headers: {
                    Authorization: `Bearer ${Cookies.get('token')}`
                }
            })
        ...
        <dialog ref={ref} className="backdrop:bg-black backdrop:opacity-75 rounded-lg">
            <div className="w-[44vw] h-[85vh] bg-white rounded-lg">
                <header className={`flex items-center justify-between p-3 ${themeValue === EThemeColor.Blue ? 'bg-dashboardCard' : 'bg-dashboardCardPink'} `}>
                    <h2 className="text-white text-xl font-semibold">Editar Oração</h2>
                ...
                <form onSubmit={handleSubmit((data) => {
                    onSubmitCreatePrayerHandler(data, id!)
